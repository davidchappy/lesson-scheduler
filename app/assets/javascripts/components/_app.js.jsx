var App = React.createClass({
  ...
  updateFromNewLessonPeriod(lessonPeriod, student, weeks) {
    ...
    if(utils.contains.call(students, student)) {
      var index;
      students.map((s, i) => {
  ...
  updateFromEditLessonPeriod(lessonPeriod, student) {
    ...
    if(isDefaultLengthChanged) {
      var weeks = utils.updateLessonLengthInWeeks(weeks, newLessonLength);
    }
  ...
  updateFromWeekChange(week) {
    ...
    var lessonPeriods = this.state.lessonPeriods;
    var lessonPeriod = utils.findElementInArrayById(lessonPeriodId, lessonPeriods);
    var index = lessonPeriods.indexOf(lessonPeriod);
    lessonPeriod.lesson_count = utils.getLessonCountFromWeeks(targetWeeks);
    lessonPeriods[index] = lessonPeriod;
  ...
  submitForm() {
    ...
    var pricing = utils.calculatePricing(this.state.lessonPeriods, this.state.allWeeks);
    $.ajax({
